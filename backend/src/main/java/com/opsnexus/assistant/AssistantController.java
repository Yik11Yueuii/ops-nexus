...
public class AssistantController {
    ...
    @PostMapping(value="/diagnose/stream",produces=MediaType.TEXT_EVENT_STREAM_VALUE) public SseEmitter diagnose(@RequestBody DiagnosisInput input,@AuthenticationPrincipal Jwt user){long userId=Long.parseLong(user.getSubject());var permit=governance.enter(userId,"DIAGNOSIS","deepseek-chat",(input.symptom()==null?0:input.symptom().length())+(input.context()==null?0:input.context().length()));var emitter=new SseEmitter(65000L);workers.submit(()->{try{send(emitter,"start",Map.of("serviceName",input.serviceName()));var result=diagnosis.diagnose(userId,input.kbId(),input.serviceName(),input.symptom(),input.context(),p->{permit.output(p.length());send(emitter,"delta",Map.of("content",p));});send(emitter,"evidence",result);send(emitter,"complete",Map.of("diagnosisId",result.get("diagnosisId"),"status","OPEN"));emitter.complete();}catch(KnowledgeException e){permit.fail(e.code);send(emitter,"error",Map.of("code",e.code,"message",e.getMessage()));emitter.complete();}catch(Exception e){permit.fail("DIAGNOSIS_FAILED");send(emitter,"error",Map.of("code","DIAGNOSIS_FAILED","message","诊断生成失败，请稍后重试"));emitter.complete();}finally{permit.close();}});return emitter;}
