...
public class AiProviderAuditService {
    ...
    public void record(AiProvider provider, String operation, boolean success, AiProviderFailure failure,
        ...
        try {
            db.update("INSERT INTO ai_provider_call_audit(provider,operation,success,failure_category,retry_count,circuit_state,latency_ms) VALUES(?,?,?,?,?,?,?)",
                provider.name(), operation, success, failure == null ? null : failure.name(), retryCount, circuitState, latencyMs);
